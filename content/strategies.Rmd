...
generate_strategy_tools <- function(devices, show_classes, title, illustration, link, link_text, link2, link2_text, what_it_can_do, why_this, price){
  ...
  cat(str_c('<a target="_blank" href="', link, '" class="btn btn-primary tool-link" role="button">', link_text, '</a>'))
  if(!is.na(link2)){
    cat(str_c('<a target="_blank" href="', link2, '" class="btn btn-primary tool-link" role="button">', link2_text, '</a>'))  
  }
  cat("</div>")
...
generate_strategy_types <- function(category, strategy, illustration, illustration_position, description){
  ...
  strategies_details %>% 
    ...
    distinct() %>% 
    ungroup() %>% 
    select(devices, show_classes, title, illustration, link, link_text, link2, link2_text, what_it_can_do, why_this, price) %>%
